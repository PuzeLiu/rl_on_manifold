...
from atacom.error_correction_wrapper import ErrorCorrectionEnvWrapper


class CircleEnvErrorCorrection(ErrorCorrectionEnvWrapper):
    def __init__(self, horizon=500, gamma=0.99, random_init=False, Kc=100., time_step=0.01):
        ...
        circle_constr = ViabilityConstraint(dim_q=2, dim_out=1, fun=self.circle_fun, J=self.circle_J, b=self.circle_b,
                                            K=0.1)
        height_constr = ViabilityConstraint(dim_q=2, dim_out=1, fun=self.height_fun, J=self.height_J, b=self.height_b,
                                            K=2)
