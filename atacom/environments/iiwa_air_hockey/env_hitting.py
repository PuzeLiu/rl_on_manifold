...
from atacom.environments.iiwa_air_hockey.env_single import AirHockeySingle


class AirHockeyHit(AirHockeySingle):
    def __init__(self, gamma=0.99, horizon=120, timestep=1 / 240., n_intermediate_steps=1,
                 debug_gui=False, env_noise=False, obs_noise=False, obs_delay=False, torque_control=True,
                 random_init=False, step_action_function=None, action_penalty=1e-3, isolated_joint_7=False):
        ...
        super().__init__(gamma=gamma, horizon=horizon, timestep=timestep,
                         ...
                         env_noise=env_noise, obs_noise=obs_noise, obs_delay=obs_delay, torque_control=torque_control,
                         step_action_function=step_action_function, isolated_joint_7=isolated_joint_7)
    ...
    def setup(self, state):
        ...
        puck_pos = np.concatenate([puck_pos, [0.0]])
        self.client.resetBasePositionAndOrientation(self._model_map['puck'], puck_pos, [0, 0, 0, 1.0])

        for i, (model_id, joint_id, _) in enumerate(self._indexer.action_data[:7]):
    ...
    def reward(self, state, action, next_state, absorbing):
        ...
        if absorbing:
            ...
            if not self.has_hit:
                ee_pos = self.get_sim_state(next_state,
                                            self.agents[0]['name'] + "/striker_mallet_tip",
