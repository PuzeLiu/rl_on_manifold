...
from atacom.environments.collision_avoidance import PointReachAtacom
from network import *


def experiment(env, alg, time_step, horizon, gamma, n_objects, random_walk, quiet, render, use_cuda,
               n_epochs=100, n_steps=10000, n_steps_per_fit=600, n_episodes_test=25,
               actor_lr=3e-4, critic_lr=3e-4, n_features=[64, 64], batch_size=64,
               lam=0.95, ent_coeff=5e-5,
               eps_ppo=0.1,
               max_kl=1e-2, n_epochs_line_search=10, n_epochs_cg=10, cg_damping=1e-2, cg_residual_tol=1e-10,
               critic_fit_params=None,
               initial_replay_size=5000, max_replay_size=200000, tau=1e-3, sigma=0.25, warmup_transitions=10000,
               lr_alpha=3e-4, target_entropy=-4,
               seed=0, results_dir='logs'):
    mdp = build_env(env, time_step, horizon, gamma, n_objects, random_walk)

    agent, build_params = build_agent(alg, mdp.info,
                                      use_cuda=use_cuda,
                                      n_epochs=n_epochs, n_steps=n_steps, n_steps_per_fit=n_steps_per_fit,
                                      n_episodes_test=n_episodes_test,
                                      actor_lr=actor_lr, critic_lr=critic_lr, n_features=n_features,
                                      batch_size=batch_size,
                                      lam=lam, ent_coeff=ent_coeff,
                                      eps_ppo=eps_ppo,
                                      max_kl=max_kl, n_epochs_line_search=n_epochs_line_search, n_epochs_cg=n_epochs_cg,
                                      ...
                                      sigma=sigma, warmup_transitions=warmup_transitions, lr_alpha=lr_alpha,
...
def build_env(env, time_step, horizon, gamma, n_objects, random_walk):
    if env == 'C':
        mdp = PointReachAtacom(time_step=time_step, horizon=horizon, gamma=gamma, n_objects=n_objects,
...
def default_params():
    defaults = dict(env='C', alg='SAC', seed=1,
                    gamma=0.99, time_step=0.01, horizon=1000, n_objects=4, random_walk=True,
                    render=False, quiet=False, use_cuda=False, results_dir='../logs/collision_avoidance')

    training_params = dict(n_epochs=100, n_steps=10000, n_steps_per_fit=600, n_episodes_test=25)
...
def parse_args():
    ...
    arg_test.add_argument('--env', choices=['C'])
    arg_test.add_argument('--alg', choices=['TRPO', 'trpo', 'PPO', 'ppo', 'DDPG', 'ddpg', 'TD3', 'td3', 'SAC', 'sac'])
    arg_test.add_argument('--time-step', type=float)
    arg_test.add_argument('--horizon', type=float)
    arg_test.add_argument('--gamma', type=float)
    arg_test.add_argument('--n-objects', type=int)
    arg_test.add_argument('--random-walk', action="store_true")
