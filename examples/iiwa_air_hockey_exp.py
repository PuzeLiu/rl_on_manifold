...
from atacom.environments.iiwa_air_hockey import AirHockeyIiwaAtacom, AirHockeyIiwaRmp
from network import *
...
def build_env(**kwargs):
    ...
    if env == '7H':
        mdp = AirHockeyIiwaAtacom(task='H', horizon=iiwa_hit_horizon, gamma=gamma, random_init=random_init,
                                  timestep=iiwa_hit_time_step, n_intermediate_steps=iiwa_hit_n_intermediate_steps,
                                  debug_gui=debug_gui)
    elif env == 'RMP':
        mdp = AirHockeyIiwaRmp(task='H', horizon=120, gamma=gamma, random_init=random_init,
...
def default_params():
    defaults = dict(env='RMP', alg='SAC', seed=1,
                    gamma=0.99, random_init=False, debug_gui=True, quiet=False, use_cuda=False,
                    iiwa_hit_time_step=1 / 240., iiwa_hit_n_intermediate_steps=4, iiwa_hit_horizon=120,
                    results_dir='../logs/iiwa_hit')
...
def parse_args():
    ...
    arg_test.add_argument('--env', choices=['7H', 'RMP'],
                          help="Environment argument ['7H', 'RMP']: "
                               "7H for Iiwa Hitting using ATACOM, "
                               "RMP for Iiwa Hitting using Riemannian Motion Policies.")
